...
- Exception: vendored skills (external skills imported via `gh skill install`, e.g. `japanese-tech-writing` / `argument-gap-edit` (Unlicense), or `grilling` (MIT, with the full license bundled at `.claude/skills/grilling/LICENSE`)) live directly under `.claude/skills/`. They have no corresponding source in `.coff/src/` and are not subject to `/coff-compile`. Re-fetch or update them with `gh skill update`. For licenses with attribution requirements such as MIT, keep the full license text and copyright notice bundled.
...
<!--{"src":".coff/src/care-giver.outputstyle.md","md5":"4826db5a7a655d55fd6892d89c51867c"} -->
