---
name: codex-delegate
description: 独立した実装、調査、バグ修正のタスクを codex@openai-codex プラグインに委譲する。
---

<!--
委譲ロジックの唯一の参照元。判断、調査、手順をここに置く。codex の隔離実行（別プロセスや別サブエージェント）は /codex:rescue が内蔵しているので、独自のサブエージェントは持たない。
-->

## いつ委譲するか

- 自己完結した実装/調査/バグ修正（メイン context を使わず別プロセスで実行したい）
