...
- 例外: vendored skill（`gh skill install` で取り込んだ外部 skill。現在は `grilling`（MIT、`.claude/skills/grilling/LICENSE` に全文を同梱）のみ）は `.claude/skills/` に直接置く。`.coff/src/` に対応するソースを持たず、`/coff-compile` の対象外。再取得や更新は `gh skill update` で行う。MIT など表示義務のあるライセンスは、ライセンス全文と著作権表示を同梱したまま保つ。
