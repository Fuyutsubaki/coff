...
`issue/` の一覧。そのまま提示する。下のブロックが実行結果に展開されていなければ、ブロック内の `$ARGUMENTS`（引数のプレースホルダ）を与えられた引数（無ければ空）に置き換えて自分で実行し、その出力を提示する。 <!-- `!` フェンスの埋め込み実行は claude 固有。codex など他 agent では LLM が実行する -->
