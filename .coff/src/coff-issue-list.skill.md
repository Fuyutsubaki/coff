...
<!--
一覧は LLM の手順にせず、埋め込みコマンド（`!` バッククォート）で bash を実行するだけにする（issue/2026/09/1413-issue-template-for-human-reader.md の判断）。埋め込みコマンドは permission が allow でないと起動が中断されるため、allowed-tools で awk を前もって許可し、パイプやループを避けて awk 1 コマンドに収めている。`issue/*/*/*.md` はシェルのグロブ展開でパス順（年月日時順）に並ぶ。状態の解釈（frontmatter または status が無ければ open）は coff-detail-issue の状態仕様に従う。要約はタイトルより後で最初に現れる、見出しでも空行でもない行。一覧ファイル（README）を置く案は陳腐化するため却下した。
-->
