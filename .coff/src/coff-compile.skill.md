...
<!--
コンパイル:
- ソースを英語にするなどで、実行時のtokenの節約を図る
- skillをソースと実行物に分けることで、管理のしやすさと実行コストの両立を試みる

lint:
- 消極的なoptimizeだ
- コンパイルで機械的に処理するとまずそうな最適化を、「ユーザーに許可を取りながらソースコードを都合の良い形式に変える」という方向で実現する
-->
...
ただし frontmatter `description` だけは別にチェックする。skill ソースでは「何をするか」と「ユーザーがどう呼ぶか（引数・呼び出し方）」以外のもの（内部手順、実装詳細、WHY）が含まれていたら短縮候補（推奨 OFF）として、削るべき箇所を引用しつつ短縮案を提示する。agent ソースの `description` はモデルがサブエージェント起動を判断する材料なので skill と同じ扱い（WHAT＋いつ起動するか以外＝内部手順・実装詳細・WHY を短縮候補・推奨 OFF）。output style ソースには引数・呼び出し方の概念がないため、WHAT（何のためのスタイルか）以外を短縮候補（推奨 OFF）として扱う。
